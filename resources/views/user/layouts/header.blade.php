<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
  ...
  <form class="d-none form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
    <div class="input-group">
      <input type="text" class="form-control bg-light border-0 small" placeholder="{{ __('adm.search_for') }}" aria-label="Search" aria-describedby="basic-addon2">
    ...
    <li class="nav-item dropdown no-arrow">
      ...
      <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
        <a class="dropdown-item text-center">
          {{ Helper::QrCodeReferrer(auth()->user()->id, 60) }}
          <button class="btn btn-success w-100 mt-2 d-block copyAffiliate" data-link="{{ route('register.form', ['r' => Helper::setReferrer(auth()->user()->id)]) }}">{{ __('adm.copy') }}</button>
        </a>  
        ...
        <a class="dropdown-item" href="{{ route('logout') }}"
              onclick="event.preventDefault();
                ...
                <i class="fas fa-sign-out-alt fa-sm fa-fw mr-2 text-gray-400"></i> {{ __('adm.logout') }}
          </a>
