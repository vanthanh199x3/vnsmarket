...
<div class="card mx-2">
  ...
  <form method="post" action="{{route('settings.update')}}">
      ...
      <div class="form-group">
        ...
        <textarea class="form-control" id="short_des" name="short_des">{{$data->short_des}}</textarea>
        @error('short_des')
        <span class="text-danger">{{$message}}</span>
        @enderror
      </div>
      {{-- <div class="form-group">
        <label for="description" >{{ __('adm.description') }} <span class="text-danger">*</span></label>
      ...
      <div class="form-group">
        <label>{{ __('adm.facebook') }}</label>
        <input type="text" class="form-control" name="phone" required value="{{$data->facebook}}">
        @error('facebook')
        <span class="text-danger">{{$message}}</span>
        @enderror
      </div>
      <div class="form-group">
        <label>{{ __('adm.zalo') }}</label>
        <input type="text" class="form-control" name="phone" required value="{{$data->zalo}}">
        @error('zalo')
        <span class="text-danger">{{$message}}</span>
        @enderror
      </div>
      <div class="form-group">
        <label>{{ __('adm.instagram') }}</label>
        <input type="text" class="form-control" name="phone" required value="{{$data->instagram}}">
        @error('instagram')
        <span class="text-danger">{{$message}}</span>
        @enderror
      </div>
      <div class="form-group">
        <label>{{ __('adm.youtube') }}</label>
      ...
      <div class="form-group">
        <label>{{ __('adm.tiktok') }}</label>
        <input type="text" class="form-control" name="youtube" required value="{{$data->tiktok}}">
        @error('tiktok')
        <span class="text-danger">{{$message}}</span>
        @enderror
      </div>
...
<script>
  $(document).ready(function() {
    ...
    //   placeholder: "{{ __('adm.short_description') }}",
    //     tabsize: 2,
    //     height: 100
    // });
