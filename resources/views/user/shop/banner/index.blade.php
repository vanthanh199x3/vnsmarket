...
@section('main-content')
 ...
 <div class="card shadow mb-4 mx-2">
    ...
    <div class="card-header">
      {{ __('adm.banner_list') }}
      <a href="{{route('shop.banner.create')}}" class="btn btn-primary btn-sm float-right" data-toggle="tooltip" data-placement="bottom" title="{{ __('adm.add').' '.__('adm.banner') }}"><i class="fas fa-plus"></i> {{ __('adm.add').' '.__('adm.banner') }}</a>
    </div>
    <div class="card-body">
          ...
          <tbody>
            @foreach($banners as $banner)   
                <tr>
                    ...
                    <td>
                        <a href="{{route('shop.banner.edit',$banner->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('shop.banner.destroy',[$banner->id])}}">
                          @csrf 
                          @method('delete')
