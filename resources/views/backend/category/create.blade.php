...
@section('title', __('adm.add').' '.__('adm.product_category'))
@section('main-content')

<div class="card mx-2">
    <h5 class="card-header">{{ __('adm.add').' '.__('adm.product_category') }}</h5>
    <div class="card-body">
      <form method="post" action="{{route('category.store')}}" id="formCategory">
        {{csrf_field()}}
        ...
        <div class="form-group">
          ...
          <input type="checkbox" name='is_parent' id='is_parent' value='1' checked> {{ __('adm.yes') }}                        
        </div>
        ...
        <div class="form-group d-none" id='parent_cat_div'>
          ...
          <select name="parent_id" class="form-control">
              ...
              @foreach($parent_cats as $key=>$parent_cat)
                <option value='{{$parent_cat->id}}'>{{ $prefix[$parent_cat->level].' '.$parent_cat->title }}</option>
                @if($parent_cat->child_cat->count())
                  @foreach($parent_cat->child_cat as $lv2)
                  <option value='{{$lv2->id}}'>{{ $prefix[$lv2->level].' '.$lv2->title }}</option>
                  @endforeach
                @endif
              @endforeach
          </select>
