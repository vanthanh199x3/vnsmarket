  ...
  <div class="card-body">
    ...
    <div class="tab-content" id="myTabContent">
        ...
        <div class="row mt-5">
          <div class="col-md-6 border rounded">
            <table class="table table-sm">
              <tbody>
                <tr>
                  <td colspan="2" class="border-0 text-uppercase text-primary">
      ...
      {{-- <div class="tab-pane fade" id="ᴡithdraᴡ" role="tabpanel" aria-labelledby="ᴡithdraᴡ-tab">
        <div class="row mt-4">
          <div class="col-md-6">
            <form id="formWithdraᴡ" action="{{ route('wallet.withdraᴡ', 1) }}" method="POST">
              @csrf 
    ...
    $('#btnTransfer').click(function(){
      if(form.valid()) {
        swal({
          ...
          text: "{{ __('adm.confirm_tranfer_message') }}" + form.find('input[name="transfer_to"]').val(),
          icon: "info",
          buttons: ["{{ __('adm.no') }}", "{{ __('adm.yes') }}"],
          // confirmButtonText: "{{ __('adm.yes') }}",
          // cancelButtonText: "{{ __('adm.no') }}",
        })
        .then((ok) => {
          if (ok) {
            form.submit();
