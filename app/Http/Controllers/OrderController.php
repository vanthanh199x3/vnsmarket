...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // if ($request->payment_method == 'dsvpay') {
            //     $orderData['payment_status'] = 'failed';
            //     $toWallet = UserWallet::masterShopWallet(1);
            //     $fromWallet = UserWallet::where(['wallet_id' => 1,'user_id' => $user->id])->first();
            //     $transferTo = User::find($toWallet->user_id);

            //     if (!empty($fromWallet) && !empty($toWallet)) {
            //         if (floatval($orderData['total_amount']) <= floatval($fromWallet->money)) {
            ...
            //                 'from'             => $user->id,
            //                 'to'               => $transferTo->id,
            //                 'money'            => $orderData['total_amount'],
            //                 'fee'              => 0,
            //                 'wallet_id'        => 1,
            //                 'content'          => 'Thanh toán đơn hàng '.$orderData['order_number'],
            //                 'status'           => 1,
            //             ];
            ...
            //                 $fromWallet->money = floatval($fromWallet->money) - floatval($orderData['total_amount']);
            //                 $fromWallet->save();

            //                 $toWallet->money = floatval($toWallet->money) + floatval($orderData['total_amount']);
            //                 $toWallet->save();
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://api.ghsv.vn/v1/order/create?',
    ...
    public function orderTrack() {
        // $orders = Order::where(['user_id' => auth()->user()->id])->orderBy('id', 'desc')->limit(10)->get();
