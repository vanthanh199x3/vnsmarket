...

{
    ...
    public function singleAddToCart(Request $request){
        ...
        $already_cart = Cart::where('user_id', auth()->user()->id)->where('order_number',null)->where('product_id', $product->id)->first();
         ...
         $id_size = $request->check_size; 
         $rs_size = Product::join('tbl_size', 'tbl_size.product_id', '=', 'products.id')
        ...
        if($already_cart) {
            ...
            $cart->brand_id = $product->brand_id;
            $cart->product_id = $product->id;
            if(!empty($rs_size->size_price_sale))
            {
                $cart->price = $rs_size->size_price_sale;
                $cart->size_order = $rs_size->size_name;
                $cart->quantity = $request->quant[1];
                $cart->amount=($rs_size->size_price_sale * $request->quant[1]);
            }
            else
            {
                $cart->price = ($product->price-($product->price*$product->discount)/100);
